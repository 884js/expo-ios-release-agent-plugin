...
設定がある場合も、各画像について次を撮影前に整理する。

- 画面状態
- 伝える魅力
- 掲載順での役割

素のアプリ画面ではなく、背景や見出しを加えた掲載画像を作る場合は、背景、色、文字、スクリーンショット配置のコンセプトを先に提示して同意を得る。合成作業はデザイン用のワークフローへ引き継ぐ。
...
ホーム画面やWidgetを撮影する場合は、掲載意図にないアプリアイコンが見えていないことを確認する。特にMaestro RunnerはDriverの再インストールでホーム画面へ戻るため、清掃後の撮影では`--no-reinstall-driver`を使う。
...
撮影ではMaestro Driverを再インストールせず、事前に整えたホーム画面を維持する。
...
撮影後は寸法検証だけで完了扱いにせず、各PNGを十分な大きさで個別に確認し、一覧でも掲載順と画面間の一貫性を確認する。次が見つかった場合は`READY`にせず、原因を除いて新しい世代へ撮り直す。

- 掲載意図にないアプリアイコンやMaestro Runner
- Expoや開発用UI、タッチ表示、通知、カーソルなどの撮影ノイズ
- 想定外のキーボード、ステータスバー、画面状態
- 文字や主要UIの欠け、不自然な余白、意図しない切り抜き
...
- 背景、見出し、端末フレームのコンセプト確認: このスキル
- 背景、見出し、端末フレームの合成: デザイン用ワークフロー
